...
namespace Deliverable
{
    public partial class CustomerRegister : Form
    {
        public CustomerRegister()
        {
            ...
            textBoxFirstname.MaxLength = 32;
            textBoxLastname.MaxLength = 32;
            textBoxPassword.MaxLength = 12;
            textBoxPhone.MaxLength = 10;
            textBoxEmail.MaxLength = 32;
            textBoxAddress.MaxLength = 40;

            //Setup comboboxes
            comboBoxGoodness.Items.Add("yes");
            comboBoxGoodness.Items.Add("no");
        ...
        private void buttonRegister_Click(object sender, EventArgs e)
        {
            ...
            string username = "", firstname = "", lastname = "", password = "", email = "", address = "", goodness="", gender="", joinDate="", dob="";
            int phone = 0, bankAccount = 0, maxRent = 0;
            ...
            try
            {
                ...
                bankAccount = int.Parse(textBoxBank.Text.Trim());
                goodness = comboBoxGoodness.Text;
                gender = comboBoxGender.Text;
                maxRent = int.Parse(textBoxRent.Text.Trim());

                //Format date
                day = dateTimePickerJoin.Value.Date.Day.ToString();
                month = dateTimePickerJoin.Value.Date.Month.ToString();
                year = dateTimePickerJoin.Value.Date.Year.ToString();
                ...
                day = dateTimePickerDob.Value.Date.Day.ToString();
                month = dateTimePickerDob.Value.Date.Month.ToString();
                year = dateTimePickerDob.Value.Date.Year.ToString();
            ...
            try
            {
                ...
                SQL.executeQuery("INSERT INTO Customer Values ('"+username+"', '"+firstname+"', '"+lastname+"', '"+password+"', " +
                    "'"+phone+"', '"+email+"', '" + address + "', '" + bankAccount + "', '" + goodness + "', '" + joinDate + "'" +
                    ", '" + dob + "', '" + gender + "', '" + maxRent + "')");

                if (SQLError.Error == "false")
                {
